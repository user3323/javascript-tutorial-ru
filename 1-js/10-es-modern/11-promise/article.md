...
Принцип очень похож на обычный `try..catch`: мы делаем асинхронную цепочку из `.then`, а затем, на том этапе, где нужно перехватить ошибки, вызываем `.catch(onRejected)`.
